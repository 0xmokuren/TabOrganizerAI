  ...
  for await (const chunk of stream) {
    ...
    if (now - lastReportAt >= 120) {
      ...
      onProgress?.({
        ...
        percent: null,
        message: 'AI が考え中…',
        detail: `${accumulated.length} 文字を受信中`,
        streamPreview: buildStreamPreview(accumulated),
      });
    }
  }
...
async function analyzeWithSession(
  ...
  return validatePlan(parsed, tabs.length, outputLanguage);
}
